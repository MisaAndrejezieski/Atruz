    ...
    try {
        ...
        favores.forEach(favor => {
            ...
            card.innerHTML = `
                ...
                <button onclick="participarFavor(${favor.id})">Participar 🤝</button>
            `;
...
// Participar de favor (aceitar favor)
async function participarFavor(favorId) {
    if (!usuarioLogado) {
        alert("Você precisa estar logado para participar de um favor.");
        return;
    }

    try {
        const resposta = await fetch("http://127.0.0.1:5000/aceitar_favor", {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({favor_id: favorId, usuario_id: usuarioLogado})
        });
        const resultado = await resposta.json();
        alert(resultado.mensagem);
        carregarFeed();
    } catch (erro) {
        console.error("Erro ao aceitar favor:", erro);
    }
}
